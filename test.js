    ...
    if (!root.items) {
        ...
        try {
            ...
            let items = dirs.map((i) => {
                ...
                i.prev = prev
                prev = i
                return i.name?.includes('.') || i.content?.link?.includes('.html') || i.execute
                    ? i
                    : loadDir(i, path)
            })
...
site: {
    ODA({
        ...
        _activate(ctrl) {
            if (!this.hash?.includes(ctrl._path)) route(ctrl._path)
        },
        ...
        setLeftDrawerFocus(item) {
            ...
            for (const ctrl of this.leftControls)
                if (ctrl._path === hash) this.selectedSiteHeaderMenu = ctrl._title
        ...
        props: {
            ...
            _editMode: {
                ...
                shared: true,
                set(x) {
                    this.$refs.iframe.contentDocument._editMode = x
                },
            },
            hash: {
                type: String,
            ...
            part: {
                ...
                get() {
                    ...
                    while (p && p.$parent && !Object.equal(p.$parent, this)) p = p.$parent
                    return p
                },
            },
            items: Array,
            ...
            _showTester() {
                return (
                    this.focusedItem &&
                    (!this.focusedItem.items || this.focusedItem.items.length === 0) &&
                    this.src
                )
            },
            showLogoImage: true,
        ...
        attached() {
            this.async(async () => {
                this.focusedItem = null
                if (this._srcIframe === 'video') this.$refs.video.muted = true
...
content: {
    ...
    ODA({
        is: 'oda-site-content-cell',
        extends: 'oda-table-cell-base',
        imports: '@oda/table, @tools/jupyter',
        template: /*html*/ `
            <style>
                :host * {
                    @apply --user-select;
        ...
        props: {
            h() {
                ...
                let style = `
                    ...
                    color: ${
                        this.item &&
                        ((this.item.name && this.item.name.includes('.md')) ||
                            (this.item.content && this.item.content.link))
                            ? '#6699cc'
                            : '#336699'
                    };
                `
        ...
        get notebook() {
            return {
                cells: [
                    {
                        cell_type: 'markdown',
                        fount: this.item?.content?.abstract
                            ? this.item.content.abstract
...
navigator: {
        ...
        props: {
            ...
            _editMode: {
                ...
                set(n) {
                    this.columns = n
                        ? [
                              { name: 'item', width: 24, template: 'oda-status-cell' },
    ...
    ODA({
        ...
        props: {
            title: {
                get() {
                    let i = this.item
                    return i
                        ? `${i.$id} [${i._statusReady.ok}/${i._statusReady.count}-${Math.round(
                              i._statusReady.percent
                          )}%]`
                        : ''
                },
            },
            _style: {
                get() {
                    return `background:conic-gradient(lightgreen 0% ${
                        this.item && this.item._statusReady.percent
                    }%, white 0% 100%);`
                },
            },
        },
        async _tap(e) {
            ...
            let s =
                e.$id.startsWith('components') && !e._statusReady
                    ? e.$id + '/$info/_info.js'
                    : 'learn/_info.js'
            this.async(() => {
                window.open(
                    ODA.rootPath +
                        '/components/editors/md-editor/md-editor.html?src=' +
                        (ODA.rootPathWhiteBook || ODA.rootPath) +
                        '/' +
                        (e.$id.includes('.') ? e.$id : s) +
                        (ODA.rootPathWhiteBook
                            ? '&_path=' + '/' + (e.$id.includes('.') ? e.$id : s)
                            : '')
                )
            })
        },
    ...
    ODA({
        is: 'oda-site-nav-cell',
        extends: 'this, oda-table-cell-base',
        imports: '@oda/table',
        template: /*html*/ `
            <style>
...
header: {
    ODA({
        ...
        getHeaderItemClass(item) {
            return {
                outline:
                    this.left.focused?.title === item.label ||
                    this.left.lastFocused?.title === item.label,
            }
        },
        attached() {
            this._resize()
    ...
    ODA({
        ...
        async _showDropdown() {
            ...
            let res = await ODA.showDropdown(
                'oda-site-menu',
                { items: this.item.items || this.items, item: this.item, mobile: this.mobile },
                { parent: this, cancelAfterLeave: true }
            )
            if (res) {
                this.ddMenuIndex = -1
    ...
    ODA({
        ...
        async search(e) {
            ...
            const queryResult = (await queryRow.json()).hits.hits.map((o) => ({
                ...
                $id: o._source.url,
                score: o._score,
                icon: o._id[0] === 'c' ? 'odant:module' : 'editor:format-align-left',
            }))
            ...
            if (e.currentTarget.value === '???') {
                ...
                function fn(l, x, goal) {
                    goal.push({
                        url: l + '/' + x.name,
                        label: x.label,
                        content: x.content && x.content.abstract ? x.content.abstract : '',
                        isLeaf: !x.folder,
                    })
                    if (x.folder)
                        x.folder.forEach((e) => {
                ...
                fn(
                    ODA.rootPath,
                    _url.default.folder.find((e) => e.name === 'learn'),
                    indLearn
                )
                fn(
                    ODA.rootPath,
                    _url.default.folder.find((e) => e.name === 'components'),
                    indCompo
                )
                ...
                async function fullCompoInd(o) {
                    ...
                    if (o.isLeaf) {
                        ...
                        _url = await import(_url)
                        let infoList = [
                            { label: 'Описание', src: './$info/description/description.md' },
                        ].concat(_url.default)
                        infoList.forEach((e) => {
                            if (e)
                                rezList.push({
                ...
                async function getContent(e) {
                    if (e.isLeaf) {
                        ...
                        let rowText = await fetch(e.url).then(function (response) {
                ...
                let indLearn0 = indLearn1
                    .map(
                        (o, i) =>
                            '{ "create" : { "_index" : "ind-learn", "_id" : "l' +
                            (i + 1) +
                            '" } }' +
                            '\n' +
                            JSON.stringify(o) +
                ...
                let indCompo0 = indCompo1
                    .map(
                        (o, i) =>
                            '{ "create" : { "_index" : "ind-learn", "_id" : "c' +
                            (i + 1) +
                            '" } }' +
                            '\n' +
                            JSON.stringify(o) +
    ...
    ODA({
        is: 'oda-seach-result',
        extends: 'oda-tree',
        props: { defaultTemplate: 'oda-site-search-cell' },
    })
    ODA({
        is: 'oda-site-search-cell',
        extends: 'oda-site-nav-cell',
        template: `
    ...
    ODA({
        ...
        props: {
            ...
            items() {
                return (
                    (this.item &&
                        (this.item.items || []).filter((i) => !(i.name && i.name.includes('.')))) ||
                    []
                )
            },
            item: {
